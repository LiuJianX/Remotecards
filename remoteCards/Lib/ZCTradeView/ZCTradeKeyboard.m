...
- (void)setupAllBtns
{
    for (int i = 0; i < ZCKeyboardBtnCount; i++) {
        ...
        if (i == 9) {  // 确定按钮
            ...
            btn.titleLabel.font = HYQIHEISIZE(ZCScreenWidth * 0.046875) ;
            [btn addTarget:self action:@selector(okBtnClick) forControlEvents:UIControlEventTouchUpInside];
        } else if (i == 10) {  // 0 按钮
            [btn setTitle:@"0" forState:UIControlStateNormal];
            btn.titleLabel.font = HYQIHEISIZE(ZCScreenWidth * 0.06875);
            //[UIFont fontWithName:@"HelveticaNeue-Light" size:ZCScreenWidth * 0.06875];
            btn.tag = 0;
            ...
            btn.titleLabel.font = HYQIHEISIZE(ZCScreenWidth * 0.046875); //[UIFont systemFontOfSize:ZCScreenWidth * 0.046875];
            [btn addTarget:self action:@selector(deleteBtnClick) forControlEvents:UIControlEventTouchUpInside];
        } else {  // 其他数字按钮
            [btn setTitle:[NSString stringWithFormat:@"%d", i + 1] forState:UIControlStateNormal];
            btn.titleLabel.font = HYQIHEISIZE(ZCScreenWidth * 0.06875); //[UIFont fontWithName:@"HelveticaNeue-Light" size:ZCScreenWidth * 0.06875];
            btn.tag = i + 1;
            [btn addTarget:self action:@selector(numBtnClick:) forControlEvents:UIControlEventTouchUpInside];
            [self.numBtns addObject:btn];
        }
